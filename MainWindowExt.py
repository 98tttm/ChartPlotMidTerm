...
class MainWindowExt(Ui_MainWindow):
    ...
    def openFileDialog(self):
        file_path, _ = QFileDialog.getOpenFileName(self.MainWindow,"Open Dataset","","Excel Files (*.xlsx *.xls)")
        if file_path:
    ...
    def openChartInBrowser(self):
        ...
        try:
            ...
            df['Tín Chỉ'] = (df['Tín Chỉ'].astype(str).str.extract(r'(\d+)', expand=False).astype(float))
            fig = px.sunburst(df,path=['Học Kỳ', 'Bắt buộc/tự chọn', 'Tên học phần'],values='Tín Chỉ',color='Học Kỳ',title="Chương trình học - Sunburst Chart")
    ...
    def saveChartToHTML(self):
        ...
        file_path, _ = QFileDialog.getSaveFileName(self.MainWindow,"Save HTML Chart","chart.html","HTML Files (*.html)")
        if not file_path:
            return
        try:
            df = pd.read_excel(dataset_path, sheet_name='Sheet1')
